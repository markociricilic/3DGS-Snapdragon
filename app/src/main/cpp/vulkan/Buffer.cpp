...
void Buffer::printBuffer(){
    std::vector<char> elems = this->download();
    std::ostringstream oss;

    for (int i = 0; i < this->size; i++) {
//      oss << "elem " << i << ": " << static_cast<int>(elems[i]);
        oss << static_cast<int>(elems[i]);
        if (i < this->size - 1) {
            oss << ", "; // Add a separator between elements
        }
    }
    // Print the entire buffer as a single log message
    LOGO("%s", oss.str().c_str());
}

void Buffer::printBufferFloats(){
    std::ostringstream oss;

    std::vector<char> elems = this->download();
    int num_floats = this->size / sizeof(float);
    auto floats = reinterpret_cast<const float *>(elems.data());
...
std::shared_ptr<Buffer> Buffer::storage(std::shared_ptr<VulkanContext> context, uint64_t size, bool concurrentSharing,
    ...
    return std::make_shared<Buffer>(context, size,
                                    ...
                                    VMA_MEMORY_USAGE_GPU_ONLY, // memory is only accessible on GPU. copy to CPU to read/debug
